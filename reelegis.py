...
# def load_data_brasil():
#     brasil = pd.read_excel('brasil.xlsx')
#     return brasil
#
# dfbrasil = load_data_brasil()
#
# br = px.bar(dfbrasil, x='media', y='label_pt', height=500, color='media',color_continuous_scale='Sunsetdark',
# labels=dict(label_pt="", media="Ênfase Temática %"), orientation='h')
#
# br.update_layout(showlegend=False, yaxis={'categoryorder': 'total ascending'})
...
if st.checkbox("E aí, quem se reelegeu?", False):

    st.title("Estatísticas eleitorais 🗳️")
    localidade = {
    'AC': [ -8.77, -70.55],
    'AL': [ -9.71, -35.73],
    #'AM': [ -3.07, -61.66],
    ...
    localidade = pd.DataFrame.from_dict(localidade, orient='index').reset_index()
    localidade.rename(columns = {'index':'UF'}, inplace = True)
    localidade.rename(columns = {0:'lat'}, inplace = True)
    localidade.rename(columns = {1:'lon'}, inplace = True)
    ...
    labels=dict(Taxa="", Porcentagem="% Reeleição"))
    figura_pizza.update_layout(showlegend=False, yaxis={'categoryorder': 'total ascending'})
    st.subheader(f"Nas eleições de 2022, **{round(taxa_de_reeleicao)}%** dos/as parlamentares concorrendo à reeleição conseguiram se reeleger")
