...
congresso = st.radio("", ['Resultados sobre Reeleição e Renovação', 'A nova Câmara dos Deputados'], key='1')

    #st.warning('Consulte nosso relatório completo [aqui](link do relatorio)')
if congresso == 'Resultados sobre Reeleição e Renovação':
    ...
    @st.cache(ttl=60*5)
    def load_data_leg57():
        rel = pd.read_excel('leg57.xlsx')
        return rel

    leg57 = load_data_leg57()

    leg57 = leg57.dropna()

    st.title('Composição da Câmara dos Deputados por gênero 🚺 🚹')
    total = leg57['nome'].unique()

    mulheres = leg57[leg57.genero == 'F'].count()
    taxa_mulheres = mulheres.iloc[0] /513 * 100

    taxa_homens = 100 -taxa_mulheres
    taxas = [['Gênero',taxa_homens],['Gênero', taxa_mulheres]]
    taxas = pd.DataFrame(taxas, columns=['Taxa', 'Porcentagem'])
    rotulos = ['% Homens', '% Mulheres']
    taxas['Taxa de'] = rotulos
    figura_genero=px.bar(taxas,x="Porcentagem",y='Taxa', title='Composição da Câmara dos Deputados por Gênero',
    orientation='h', color_continuous_scale='Tealgrn',color='Taxa de',
    color_discrete_map={"% Homens": '#5DBB63',
    "% Mulheres": '#710193'},
    labels=dict(Taxa="", Porcentagem="%"))
    figura_genero.update_layout(showlegend=True, yaxis={'categoryorder': 'total ascending'})
    figura_genero.update_traces(width=.6)
    figura_genero.update_layout(yaxis_visible=False, yaxis_showticklabels=False)
    #st.info(f'Dos **{total_reeleicao}** parlamentares que concorreram à reeleição, **{round(taxa_de_reeleicao)}%** conseguiram uma cadeira na Câmara dos Deputados. Esse resultado representa **{round(taxa_de_reeleicao_geral)}%** das **513** cadeiras da Câmara dos Deputados.')
    #st.info(f'**{round(taxa_de_reeleicao)}%** dos parlamentares que concorreram à reeleição conseguiram uma cadeira na Câmara dos Deputados. Esse resultado representa **{round(taxa_de_reeleicao_geral)}%** das 513 cadeiras da Câmara dos Deputados.') #A Câmara dos Deputados foi renovada em **{round(taxa_de_renovacao)}%**.')
    #st.info(f"Nas eleições de 2022, **{round(taxa_de_reeleicao)}%** dos parlamentares concorrendo à reeleição conseguiram se reeleger. Consequentemente, o Congresso Nacional foi renovado em **{round(taxa_de_renovacao)}%**.")
    figura_genero.update_layout(legend=dict(
        orientation="h",
        yanchor="bottom",
        y=1.02,
        xanchor="right",
        x=1), legend_title_text='')
    figura_genero.add_vline(x=50, line_dash="dash", line_color="red")
    st.plotly_chart(figura_genero, use_container_width=True)

    st.title('Composição da Câmara dos Deputados por cor/raça 👤')

    branca = leg57[leg57['raça'] == 'branca'].count()
    branca_geral = branca.iloc[0] /513 * 100
    preta = leg57[leg57['raça'] == 'preta'].count()
    preta_geral = preta.iloc[0] /513 * 100
    parda = leg57[leg57['raça'] == 'parda'].count()
    parda_geral = parda.iloc[0] /513 * 100
    indigena = leg57[leg57['raça'] == 'indígena'].count()
    indigena_geral = indigena.iloc[0] /513 * 100
    sem = leg57[leg57['raça'] == 'sem informação'].count()
    sem_info = sem.iloc[0] /513 * 100

    #brancos = 100 -branca_geral
    taxas = [['cor',branca_geral],['cor', parda_geral], ['cor', preta_geral],
    ['cor', indigena_geral], ['cor', sem_info]]
    taxas = pd.DataFrame(taxas, columns=['Taxa', 'Porcentagem'])
    rotulos = ['% Branca', '% Parda', '% Preta', '% Indígena', '% Sem Informação']
    taxas['Taxa de'] = rotulos
    figura_cor=px.bar(taxas,x="Porcentagem",y='Taxa', title='Composição da Câmara dos Deputados por cor/raça',
    orientation='h', color_continuous_scale='Tealgrn',color='Taxa de',
    color_discrete_map={"% Branca": 'darkcyan',
    "% Parda": 'orange',
    "% Preta": '#87CEEB',
    "% Indígena" : '#FF1493',
    "% Sem Informação": '#000000'},
    labels=dict(Taxa="", Porcentagem="%"))
    figura_cor.update_layout(showlegend=True, yaxis={'categoryorder': 'total ascending'})
    figura_cor.update_traces(width=.6)
    figura_cor.update_layout(yaxis_visible=False, yaxis_showticklabels=False)
    #st.info(f'Dos **{total_reeleicao}** parlamentares que concorreram à reeleição, **{round(taxa_de_reeleicao)}%** conseguiram uma cadeira na Câmara dos Deputados. Esse resultado representa **{round(taxa_de_reeleicao_geral)}%** das **513** cadeiras da Câmara dos Deputados.')
    #st.info(f'**{round(taxa_de_reeleicao)}%** dos parlamentares que concorreram à reeleição conseguiram uma cadeira na Câmara dos Deputados. Esse resultado representa **{round(taxa_de_reeleicao_geral)}%** das 513 cadeiras da Câmara dos Deputados.') #A Câmara dos Deputados foi renovada em **{round(taxa_de_renovacao)}%**.')
    #st.info(f"Nas eleições de 2022, **{round(taxa_de_reeleicao)}%** dos parlamentares concorrendo à reeleição conseguiram se reeleger. Consequentemente, o Congresso Nacional foi renovado em **{round(taxa_de_renovacao)}%**.")
    figura_cor.update_layout(legend=dict(
        orientation="h",
        yanchor="bottom",
        y=1.02,
        xanchor="right",
        x=1), legend_title_text='')
    figura_cor.add_vline(x=50, line_dash="dash", line_color="red")
    st.plotly_chart(figura_cor, use_container_width=True)


    st.title('Composição da Câmara dos Deputados por Partido 🏛️')
    partidos = leg57.groupby(by=['partido']).sum()
    partidos['porcentagem'] = partidos['incumbente']/513 * 100
    partidos = pd.DataFrame(data=partidos).sort_values(by = ['porcentagem'],
    ascending=False).reset_index()
    partidos['cor_partido'] = '% ' + partidos['partido']

    #brancos = 100 -branca_geral
    taxas['Taxa de'] = rotulos
    estado_parla = px.bar(partidos, x='porcentagem', y='partido', height=500, color='porcentagem',
            #color_continuous_scale=px.colors.sequential.Viridis,
    color_continuous_scale='Burg',
            # site com as cores: https://plotly.com/python/builtin-colorscales/
    labels=dict(partido="", porcentagem="% Composição da Câmara"), orientation='h')
    estado_parla.update_layout(showlegend=False, yaxis={'categoryorder': 'total ascending'})
    st.plotly_chart(estado_parla, use_container_width=True)


    st.title('Fragmentação Partidária')
    @st.cache(ttl=60*5)
    def load_data_frag():
        rel = pd.read_excel('fragmentacaolegistiva.xlsx')
        return rel

    fragmentacao = load_data_frag()

    fragmentacao = fragmentacao.dropna()

    # st.write(fragmentacao)
    # fig = px.line(fragmentacao, x='Ano', y='NEP').add_traces(px.bar(
    # fragmentacao,  x='Ano', y='NEP'))
    # fig.update_layout(xaxis={"dtick":1},margin={"t":0,"b":0},height=500, width=700)
    fig=px.bar(
    fragmentacao, x="Ano", y='NEP', text='NEP',
    labels=dict(Ano="", NEP="Número Efetivo de Partidos (NEP)")).update_layout(xaxis={"dtick":1})#,margin={"t":0,"b":0},height=400, width=700)
    ...
    @st.cache(ttl=60*5)
    def load_data_rele():
        rele = pd.read_excel('reeleicao_renovacao.xlsx')
        return rele
    ...
    # fragmentacao,  x='Ano', y='NEP'))
    # fig.update_layout(xaxis={"dtick":1},margin={"t":0,"b":0},height=500, width=700)
    figura_pizza=px.bar(rele,x="%",y='Ano', height=500,
    orientation='h',color='Status',
    color_discrete_map={"Reeleição": '#21ADA8',
    "Renovação": 'orange'},
    labels=dict(Ano="Ano"))
    figura_pizza.update_yaxes(type='category')
    #figura_pizza.update_layout(showlegend=True, yaxis={'categoryorder': 'total ascending'})
    #figura_pizza.update_traces(width=.6)
    #figura_pizza.update_layout(yaxis_visible=True, yaxis_showticklabels=True)
    #figura_pizza.update_layout(yaxis={'type': 'linear'})
    #st.info(f'**{round(taxa_de_reeleicao)}%** dos parlamentares que concorreram à reeleição conseguiram uma cadeira na Câmara dos Deputados. Esse resultado representa **{round(taxa_de_reeleicao_geral)}%** das 513 cadeiras da Câmara dos Deputados.') #A Câmara dos Deputados foi renovada em **{round(taxa_de_renovacao)}%**.')
    #st.info(f"Nas eleições de 2022, **{round(taxa_de_reeleicao)}%** dos parlamentares concorrendo à reeleição conseguiram se reeleger. Consequentemente, o Congresso Nacional foi renovado em **{round(taxa_de_renovacao)}%**.")
    # figura_pizza.update_layout(legend=dict(
    #     orientation="h",
